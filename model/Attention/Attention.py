...
from model.Attention.Attention_modules import Encoder_blocks
    
class TF_Attention(nn.Module):
    def __init__(self, backbone1: nn.Module, backbone2: nn.Module, backbone_freezing: bool=True,
                 cross_attention: bool=False,inversed_cross_attention: bool=False,shared_attention:bool=False,
...
# backbone3: nn.Module=None,
        ...
        # Cross Attention
        self.TF_layers = Encoder_blocks(emb_size=emb_size, cross_attention=cross_attention,shared_attention=shared_attention,
                    depth=depth,inversed_cross_attention=inversed_cross_attention,
                    heads= heads,dropout=dropout,last_gelu=last_gelu)

        self.avgpool = nn.AdaptiveAvgPool1d((1))

        self.fc = nn.Linear(emb_size * 2, class_num) # emb_size * 4 ? 
    ...
    def forward(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        tf_output1, tf_output2 = self.TF_layers(out1,out2)
        ...
        combined = torch.cat((tf_output1, tf_output2), dim = -1) # [batch, emb_size*2]
        fc_out = self.fc(combined)
                
        return fc_out
    
class MHDA_Attention(nn.Module): # 입력이 2개일 때 MHDA 모델
    def __init__(self, backbone1: nn.Module, backbone2: nn.Module, backbone_freezing: bool=True,
                 inversed_cross_attention: bool=False,shared_attention:bool=False,
                    backbone_output_dim: int=2048,emb_size: int=768, num_patches: int=196,depth: int=1,
                    positional_embedding: bool=True,cls_token: bool=False,last_gelu=False,
                    heads: int = 8,dropout:float=0.,emb_dropout:float=0., class_num=2):
        ...
        self.dropout = nn.Dropout(emb_dropout)
        # Cross Attention
        self.MHSA_layers = Encoder_blocks(emb_size=emb_size,  cross_attention=False,shared_attention=shared_attention,
                    depth=depth,inversed_cross_attention=inversed_cross_attention,
                    heads= heads,dropout=dropout,last_gelu=last_gelu)
        self.MHCA_layers = Encoder_blocks(emb_size=emb_size, cross_attention=True,shared_attention=shared_attention,
                    depth=depth,inversed_cross_attention=inversed_cross_attention,
                    heads= heads,dropout=dropout,last_gelu=last_gelu)

        self.avgpool = nn.AdaptiveAvgPool1d((1))

        self.fc = nn.Linear(emb_size * 4, class_num) # emb_size * 4 ? 
    ...
    def forward(self, x: Tensor, y: Tensor) -> Tensor:
        ...
        MHSA_output1, MHSA_output2 = self.MHSA_layers(out1,out2)
        MHCA_output1, MHCA_output2 = self.MHCA_layers(out1,out2)
        ...
        if self.cls_token:
            MHSA_output1 = MHSA_output1[:,:,0] # 인덱싱
        ...
        combined = torch.cat((MHSA_output1, MHSA_output2,MHCA_output1,MHCA_output2), dim = -1) # [batch, emb_size*2]
        fc_out = self.fc(combined)
