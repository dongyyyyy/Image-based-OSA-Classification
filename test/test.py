...
from model.Cross_Attention.Cross_Attention_modules import backbone_load_weight
from model.Cross_Attention.Cross_Attention import TF_Cross_Attention_without_MLP, TF_Cross_Attention
from model.Cross_Attention.Cross_Attention_init import TF_Cross_Attention_init, TF_Cross_Attention_init_osh, TF_Cross_Attention_init_osh_origin
from model.Self_Attention.Self_Attention import TF_Attention_without_MLP, TF_Attention
import logging

from torchsummary import summary
from torch.optim.lr_scheduler import LambdaLR
# from utils.dataloader import AHI_classification
from utils.dataloader import AHI_multi
...
if __name__ == '__main__':
    ...
    for fold in range(args.k_fold):
        ...
        backbone1 = resnet50_without_GAP(class_num=args.num_classes)
        backbone2 = resnet50_without_GAP(class_num=args.num_classes)
        
        if fold + 1 == 1:
            model_weight_path_1 = f'{args.pretrained_model_weight_path}Training_view_1/k_fold_{args.k_fold}_num_fold_{fold}_seed_{args.seed}_rotationDegree_{args.rotate_degree}_img_size_{args.image_size}/best_model.pth.tar'
            model_weight_path_2 = f'{args.pretrained_model_weight_path}Training_view_1/k_fold_{args.k_fold}_num_fold_{fold}_seed_{args.seed}_rotationDegree_{args.rotate_degree}_img_size_{args.image_size}/best_model.pth.tar'
            
            #########################################################
            # backbone load weight
            backbone1 = backbone_load_weight(model=backbone1,model_weight_path=model_weight_path_1)
            backbone2 = backbone_load_weight(model=backbone2,model_weight_path=model_weight_path_2)
            num_patches = args.image_size // 32 * args.image_size // 32
            #########################################################
            if args.model_architecture == 'version0':
                model = TF_Cross_Attention_without_MLP(backbone1 = backbone1, backbone2 = backbone2,
                            backbone_freezing=args.backbone_freezing,
                            backbone_output_dim=args.backbone_output_dim, 
                            emb_size = args.emb_size,num_patches=num_patches,
                            inversed_cross_attention=args.inversed_cross_attention, heads = args.num_heads,
                            positional_embedding=args.positional_embedding,cls_token=args.cls_token,skip_connection=args.skip_connection,
                            class_num=args.num_classes)
            elif args.model_architecture == 'version1':
                model = TF_Cross_Attention(backbone1 = backbone1, backbone2 = backbone2,
                        backbone_freezing=args.backbone_freezing,
                        backbone_output_dim=args.backbone_output_dim, 
                        emb_size = args.emb_size,num_patches=num_patches,
                        inversed_cross_attention=args.inversed_cross_attention, heads = args.num_heads, depth=args.depth,
                        positional_embedding=args.positional_embedding,cls_token=args.cls_token,dropout=args.dropout,emb_dropout=args.emb_dropout,shared_attention=args.shared_attention,
                        class_num=args.num_classes)
                print(f'args.shared_attention = {args.shared_attention}')
            elif args.model_architecture == 'version1_init':
                model = TF_Cross_Attention_init(backbone1 = backbone1, backbone2 = backbone2,
                    backbone_freezing=args.backbone_freezing,
                    backbone_output_dim=args.backbone_output_dim, 
                    emb_size = args.emb_size,num_patches=num_patches,
                    inversed_cross_attention=args.inversed_cross_attention, heads = args.num_heads, depth=args.depth,
                    positional_embedding=args.positional_embedding,cls_token=args.cls_token,dropout=args.dropout,emb_dropout=args.emb_dropout,last_gelu=args.last_gelu,
                    class_num=args.num_classes)
            elif args.model_architecture == 'version1_osh':
                model = TF_Cross_Attention_init_osh(backbone1 = backbone1, backbone2 = backbone2,
                    backbone_freezing=args.backbone_freezing,
                    backbone_output_dim=args.backbone_output_dim, 
                    emb_size = args.emb_size,num_patches=num_patches,
                    inversed_cross_attention=args.inversed_cross_attention, heads = args.num_heads, depth=args.depth,
                    positional_embedding=args.positional_embedding,cls_token=args.cls_token,dropout=args.dropout,emb_dropout=args.emb_dropout,last_gelu=args.last_gelu,
                    class_num=args.num_classes)
            elif args.model_architecture == 'version1_osh_origin':
                model = TF_Cross_Attention_init_osh_origin(backbone1 = backbone1, backbone2 = backbone2,
                        backbone_freezing=args.backbone_freezing,
                        backbone_output_dim=args.backbone_output_dim, 
                        emb_size = args.emb_size,num_patches=num_patches,
                        inversed_cross_attention=args.inversed_cross_attention, heads = args.num_heads, depth=args.depth,
                        positional_embedding=args.positional_embedding,cls_token=args.cls_token,dropout=args.dropout,emb_dropout=args.emb_dropout,last_gelu=args.last_gelu,
                        class_num=args.num_classes)
            elif args.model_architecture == 'self_attention':
                model = TF_Attention(backbone1 = backbone1, backbone2 = backbone2,
                        backbone_freezing=args.backbone_freezing,
                        backbone_output_dim=args.backbone_output_dim, 
                        emb_size = args.emb_size,num_patches=num_patches,
                        heads = args.num_heads, depth=args.depth,
                        positional_embedding=args.positional_embedding,cls_token=args.cls_token,dropout=args.dropout,emb_dropout=args.emb_dropout,last_gelu=args.last_gelu,
                        class_num=args.num_classes)
            ###############################################################################################
        ...
        val_dataset = AHI_multi(args=args, folder_list = image_validation_list_with_label, folder_label = image_validation_list_with_label, image_view = args.image_view,  size=448,is_train=False)
        ...
        with tqdm(validationloader,desc='Val',unit='batch') as tepoch:
            for index,(batch_image, batch_label,_) in enumerate(tepoch):
                ...
                batch_image = (batch_image_1, batch_image_2)
                
                batch_label = batch_label.cuda()
                
                with torch.no_grad():
                    # logit_output = model(batch_image)
                    logit_output = model(batch_image_1, batch_image_2)
