...
if __name__ == '__main__':
    ...
    for fold in range(args.k_fold):
        ...
        model_1 = models_comparison_single_view(model_architecture=args.backbone_architecture,num_classes=args.num_classes,pretrained=True,image_size=args.image_size)
        model_2 = models_comparison_single_view(model_architecture=args.backbone_architecture,num_classes=args.num_classes,pretrained=True,image_size=args.image_size)
        

        current_fold_path_1 = args.model_1_path + f'/fold_{fold+1}/'
        current_fold_path_2 = args.model_2_path + f'/fold_{fold+1}/'

        model_weight_path_1 = os.path.join(current_fold_path_1,'best_model.pth.tar')
        model_weight_path_2 = os.path.join(current_fold_path_2,'best_model.pth.tar')

        # model weight load
        model_1.load_state_dict(torch.load(model_weight_path_1,weights_only=True)['model_state_dict'])
        model_2.load_state_dict(torch.load(model_weight_path_2,weights_only=True)['model_state_dict'])

        model_1 = model_1.cuda()
        model_2 = model_2.cuda()
        # summary(model, input_size=(3, 448, 448))

        test_file_path = os.path.join(current_fold_path_1,'test_filename.txt')
        ...
        with tqdm(validationloader,desc='Val',unit='batch') as tepoch:
            for index,(batch_image, batch_label,_) in enumerate(tepoch):
                ...
                with torch.no_grad():
                    ...
                    logit_output_1 = model_1(batch_image_1)
                    logit_output_2 = model_2(batch_image_2)
                    
                    
                    _,predict = torch.max(logit_output_1+logit_output_2,1)
