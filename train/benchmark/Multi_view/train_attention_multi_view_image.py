...
from model.Attention.Attention import TF_Attention, MHDA_Attention
...
if __name__ == '__main__':
    ...
    for fold in range(args.k_fold):
        ...
        if args.MHDA: 
            model = MHDA_Attention(backbone1=backbone1, backbone2=backbone2, backbone_freezing=args.backbone_freezing,
                                inversed_cross_attention=args.inversed_cross_attention,
            ...
            model = TF_Attention(backbone1=backbone1, backbone2=backbone2, backbone_freezing=args.backbone_freezing,shared_attention=args.shared_attention,
                                inversed_cross_attention=args.inversed_cross_attention, cross_attention=args.cross_attention,
                                backbone_output_dim=output_dim1,emb_size=args.emb_size, num_patches=num_patches,depth=args.depth,
                                positional_embedding=args.positional_embedding,cls_token=args.cls_token,last_gelu=args.last_gelu,
                                heads=args.num_heads,dropout=args.dropout,emb_dropout=args.emb_dropout, class_num=args.num_classes)
        ...
        image_train_list = [item.strip().replace('/data/', '/home/oem/') for item in image_train_list]
        ...
        train_dataset = AHI_multi(args=args, folder_list = image_train_list_with_label, folder_label = image_train_list_with_label, image_view = args.image_view, size=args.image_size,is_train=True)
        val_dataset = AHI_multi(args=args, folder_list = image_validation_list_with_label, folder_label = image_validation_list_with_label, image_view = args.image_view,  size=args.image_size,is_train=False)
